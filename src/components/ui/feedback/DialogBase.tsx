...
>(
  (
    ...
    ).onWheel as undefined | ((e: React.WheelEvent) => void);
    ...
    return (
      <DialogPortalBase>
        ...
        >
          {children}
          <DialogPrimitive.Close className="absolute right-2 top-2 sm:right-4 sm:top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground z-10 touch-manipulation p-1">
            <XIcon className="h-5 w-5 sm:h-4 sm:w-4 hover:text-red-500 font-extrabold" />
