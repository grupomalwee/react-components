...
>(({ className, children, leftIcon, showCaret = true, ...props }, ref) => {
  return (
    <CollapsiblePrimitive.CollapsibleTrigger
      ref={ref}
      className={cn(
        "flex w-full items-center justify-between p-3 text-left font-medium transition-all duration-500 ease-ease-standard hover:bg-accent hover:text-accent-foreground rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 [&[data-state=open]_.caret-icon]:rotate-180",
        className
      )}
    ...
    >
      ...
      {showCaret && (
        <span className="caret-icon flex-shrink-0 transition-transform duration-500 ease-ease-standard">
