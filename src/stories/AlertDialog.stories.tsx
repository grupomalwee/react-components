...
export const TesteCancelarExclusao: Story = {
  args: {
    triggerText: "Excluir Conta",
    title: "Tem certeza absoluta?",
    description:
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Abrir o dialog de exclusão", async () => {
      const triggerButton = canvas.getByTestId("alert-trigger");
      await userEvent.click(triggerButton);
    ...
    await step("Testar hover no botão de cancelar", async () => {
      const cancelButton = document.querySelector(
        '[data-testid="alert-cancel"]'
      );
      if (cancelButton) {
        await userEvent.hover(cancelButton as HTMLElement);
        expect(cancelButton).toBeInTheDocument();
        expect(cancelButton?.textContent).toBe("Cancelar");
      }
    });

    await step("Verificar que o botão de excluir está presente", async () => {
...
export const TesteFluxoCompleto: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Executar testes básicos de abertura", async () => {
      const triggerButton = canvas.getByTestId("alert-trigger");
      await userEvent.click(triggerButton);
    ...
    await step("Testar interação adicional com os botões", async () => {
      const cancelButton = document.querySelector(
        '[data-testid="alert-cancel"]'
      );
      ...
      if (actionButton) {
        await userEvent.hover(actionButton as HTMLElement);
        expect(actionButton).toBeInTheDocument();
        expect(actionButton?.textContent).toBe("Prosseguir");
      }
    });
  },
};
