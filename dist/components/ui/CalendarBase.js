...
function CalendarBase({ className, classNames, showOutsideDays = true, ...props }) {
    return (_jsx(DayPicker, { showOutsideDays: showOutsideDays, className: cn("bg-zinc-50 p-3", className), classNames: {
            months: "flex items-center flex-col sm:flex-row space-y-4 sm:space-x-4 sm:space-y-0",
            month: "space-y-4",
            caption: "flex justify-center pt-1 relative items-center",
            caption_label: "text-sm font-medium",
            nav: "space-x-1 flex items-center",
            nav_button: cn(_jsx(ButtonBase, { variant: "outline" }), "h-7 w-7 bg-transparent p-0  opacity-50 hover:opacity-100"),
            ...
            day: cn(_jsx(ButtonBase, { variant: "ghost" }), "h-9 w-9 p-0 font-normal aria-selected:opacity-100"),
