...
export type FilterTypes = "string" | "number" | "boolean" | "date" | "select" | "multi-select";
...
export type FilterConditions = "$eq" | "$ne" | "$gt" | "$lt" | "$gte" | "$lte" | "$startsWith" | "$endsWith" | "$contains" | "$in" | "$nin" | "$exists" | "$notExists";
